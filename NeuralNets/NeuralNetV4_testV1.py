...
#parameters for inputs to all other hidden layer activation functions
for layer in range(1,n_layers):
    alpha.append(np.random.normal(size=(M[layer-1]+1)*M[layer]).reshape(M[layer-1]+1,M[layer]))
#parameters for inputs to output layer
alpha.append(np.random.normal(size=(M[layer]+1)*K).reshape(M[layer]+1,K))
