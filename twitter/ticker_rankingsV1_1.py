...
cond_l=(DF_unique[11]>=left_window)
cond_r=(DF_unique[11]<right_window)
...
for x in DF_tick_count_aug_m.index:
    ...
    x_indices=[j for j in DF_aug.index if j in tick_indices[x]]
...
for name in DF_sent_pos_aug.index:
    df_yahoo = pd.io.data.get_data_yahoo(name,start=left_window,end=right_window)
    N=len(df_yahoo)
    ret.append(df_yahoo.ix[N-1,'Adj Close']/df_yahoo.ix[0,'Open']-1)

#add monthly returns to sentiment dataframes
DF_sent_pos_aug['RET']=ret
